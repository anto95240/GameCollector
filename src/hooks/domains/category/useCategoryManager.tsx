...
export const useCategoryManager = () => {
  ...
  const getCategoryLabel = () => {
    ...
    return (labels as any)[selectedCategory] || 'Catégorie'
  }
  ...
  const handleEdit = (item: any) => {
    ...
    const name =
      item.genre_name || item.platform_name || item.status_name || item.tag_name || item.name
  ...
  const handleDelete = async (id: any) => {
    if (
      !window.confirm(t('common.confirmDelete') || 'Voulez-vous vraiment supprimer cet élément ?')
    )
      return
