...
export const useTagsManager = (initialTags = []) => {
  ...
  useEffect(() => {
    if (tagInput.trim() === '') {
      ...
      const suggestions = availableTags.filter(
        (tag) => tag.tag_name?.toLowerCase().includes(lowerInput) && !selectedTags.includes(tag._id)
      )
  ...
  const addTag = async (tagVal) => {
    ...
    if (typeof tagVal === 'object') {
      ...
      if (!trimmed) return
      tagToAdd = availableTags.find((t) => t.tag_name?.toLowerCase() === trimmed.toLowerCase())
