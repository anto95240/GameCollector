...
export const useGameFiltering = (initialGames: any[]) => {
  const {
    searchTerm,
    setSearchTerm,
    selectedFilters,
  ...
  const handleSelectFilter = (category: string, option: string | string[], mergeMulti = false) => {
    ...
    if (isSingleValueFilter) {
      ...
      const prefix = `${cat}: `
      const nextFilters = selectedFilters.filter((s: any) => !s.startsWith(prefix))
    ...
    if (mergeMulti) {
      ...
      const prefix = `${cat}: `
      const existing = selectedFilters.find((s: any) => s.startsWith(prefix))

      if (existing) {
        const existingValue = existing.split(': ')[1] || ''
        const set = new Set(existingValue.split('|').filter(Boolean))
        if (Array.isArray(option)) {
          option.forEach((o: any) => set.add(o))
        } else {
          set.add(option)
        }
        const newTag = `${cat}: ${Array.from(set).join('|')}`
        setSelectedFilters((prev: any) => prev.map((p: any) => (p.startsWith(prefix) ? newTag : p)))
        setPage(1)
        return
      }
      ...
      setSelectedFilters((prev: any) => [...prev, `${cat}: ${val}`])
      setPage(1)
      return
    }
    ...
    if (!selectedFilters.includes(newTag)) {
      setSelectedFilters((prev: any) => [...prev, newTag])
      setPage(1)
    }
  }
