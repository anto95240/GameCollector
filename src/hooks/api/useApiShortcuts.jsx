...
export const useApiShortcuts = () => {
  ...
  const updateShortcut = useCallback(
    async (actionId, newBinding) => {
      ...
      try {
        ...
        const existingIndex = currentShortcuts.findIndex((s) => s.action === actionId)
    ...
    async (actionId) => {
      ...
      try {
        ...
        if (existingIndex >= 0) {
          ...
          const defaultShortcut = getHardcodedDefaults().find((d) => d.action === actionId)
          updatedShortcuts.push({
            action: actionId,
  ...
  const resetShortcut = useCallback(
    async (actionId) => {
      ...
      setError(null)
      try {
        const updatedShortcuts = (shortcuts || []).filter((s) => s.action !== actionId)
