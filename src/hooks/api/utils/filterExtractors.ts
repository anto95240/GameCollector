export const normalizeUserId = (user: any) => user?.uid || user?._id || user?.id || null

export const parseNumeric = (value: any) => {
  const parsed = Number(value)
  return Number.isFinite(parsed) ? parsed : null
}

export const extractFilterValues = (selectedFilters: any) => {
  const values: {
    genre: string[]
    platform: string[]
