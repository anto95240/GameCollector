...
const SectionGameCard = () => {
  ...
  const activeId = useActiveOnScroll(scrollRef, '.observer-item', recentGames as never[])
  ...
  const scroll = useCallback((direction: 'left' | 'right') => {
    if (scrollRef.current) {
      ...
      const scrollAmount = itemNode
        ? (itemNode as HTMLElement).offsetWidth + (window.innerWidth <= 768 ? 15 : 20)
        : 230
      if (direction === 'left')
      ...
      {isLoading ? (
        ...
        <div className="games-carousel">
          ...
          <div className="game-cards-container" ref={scrollRef}>
            {recentGames.map((game: any, index: any) => (
              <div
                key={game._id || index}
