...
export interface TopGamesWidgetProps {
  stats: any;
  metadata?: {
    platforms?: any[];
    genres?: any[];
...
const TopGamesWidget: React.FC<TopGamesWidgetProps> = ({ stats, metadata = {} }: any) => {
  if (!stats || !stats.topGames) return null

  const topGames = stats.topGames.map((g: any) => ({
    ...g,
    platformName: metadata.platforms?.find((p: any) => p._id === g.platform)?.platform_name || 'Inconnu',
    genreName: metadata.genres?.find((mg: any) => mg._id === g.genre)?.genre_name || '',
