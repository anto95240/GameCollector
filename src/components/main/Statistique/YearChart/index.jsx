...
const CustomTooltip = ({ label, value, position }) => {
  ...
  return (
    <div className="year-custom-tooltip" style={{ left: position.x, top: position.y }}>
...
const YearChart = ({ games = [], metadata = {} }) => {
  ...
  const processedData = useMemo(() => {
    ...
    if (viewType === "year") {
      const yearCounts = {};
      games.forEach((g) => {
        if (g.year) yearCounts[g.year] = (yearCounts[g.year] || 0) + 1;
      });
        ...
        for (let curr = y; curr <= y + 9; curr++) {
          if (yearCounts[curr]) {
            dataByPeriod[periodLabel].push({
              name: curr.toString(), count: yearCounts[curr], color: "#0068AC",
            });
          }
        }
      }
      ...
      games.forEach((g) => {
        g.tags_ids?.forEach((tagObj) => {
          ...
          if (exactMeta) {
            ...
            const customMeta = metadata.tags?.find((t) => (t.tag_name || t.name) === tagName && t.user_id);
            const bestMeta = customMeta || exactMeta;

            if (!tagCounts[tagName]) {
              tagCounts[tagName] = { count: 0, color: bestMeta.color || "#5AF2FF" };
            }
  ...
  const handleBarEnter = (data, index) => {
    ...
    setTooltipData({ label: itemPayload.name, value: itemPayload.count || data.value });
  };
    ...
    <div className="year-chart-wrapper">
      <div className="year-chart-content">
        <div className="year-chart-header">
          <CustomSelect options={viewOptions} value={viewType} onChange={setViewType} />
          {processedData?.options?.length > 0 && (
            <CustomSelect options={processedData.options} value={period} onChange={setPeriod} />
          )}
        </div>
          ...
          {currentData.length === 0 ? (
            ...
            <>
                ...
                <BarChart data={currentData} margin={{ top: 30, right: 0, left: 0, bottom: 0 }}>
                  ...
                  <Bar dataKey="count" radius={[4, 4, 0, 0]} barSize={35} onMouseEnter={handleBarEnter} onMouseLeave={handleBarLeave}>
                    {currentData.map((entry, index) => (
                      <Cell key={`cell-${index}`} className={getBarClass(index)} fill={entry.color} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              {activeIndex !== null && tooltipPos && tooltipData && (
                <CustomTooltip label={tooltipData.label} value={tooltipData.value} position={tooltipPos} />
              )}
            </>
          )}
        </div>
