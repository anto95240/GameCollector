...
export interface RadarGenreChartProps {
  stats: any;
  metadata?: {
    genres?: any[];
    [key: string]: any;
...
const RadarGenreChart: React.FC<RadarGenreChartProps> = ({ stats, metadata = {} }: any) => {
  const data = useMemo(() => {
    if (!stats || !stats.radar) return []

    return stats.radar.map((r: any) => {
      const meta = metadata.genres?.find((m: any) => m._id === r.subject)
      return {
