...
const DetailFooter = () => {
  ...
  useEffect(() => {
    const fetchSuggestedGames = async () => {
      ...
      try {
        ...
        const filteredGames = (historyGames || []).filter(
          (g: any) => g && !isSameGame(g, currentIdentifier)
        )
  ...
  const activeId = useActiveOnScroll(scrollRef, '.observer-item', suggestedGames as never[])
  ...
  return (
    <footer className="footer-section">
      ...
      {isLoading ? (
        <p className="loading-text" style={{ textAlign: 'center', padding: '20px 0' }}>
          Chargement...
        </p>
        ...
        <div className="detail-carousel mx-auto">
          ...
          <div className="recent-scroll no-scrollbar" ref={scrollRef}>
            {suggestedGames.map((game: any, index: any) => {
        ...
        <p className="noGame">Aucun autre jeu disponible.</p>
      )}
    </footer>
  )
}
