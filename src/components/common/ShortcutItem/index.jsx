  ...
  return (
    <div className={`keyboard-help-item ${item.isCustomized ? 'customized' : ''} ${!item.isEnabled ? 'disabled' : ''} ${isEditing ? 'editing' : ''}`}>
        ...
        {isEditing ? (
          ...
          <>
            ...
            <span className="keyboard-help-description">
              {item.description}
              {item.isCustomized && <span className="custom-badge">{t('keyboardShortcuts.customBadge')}</span>}
              {!item.isEnabled && <span className="disabled-badge">{t('keyboardShortcuts.disabledBadge')}</span>}
            </span>
          </>
        )}
      </div>
      <div className="keyboard-help-actions">
        {isEditing ? (
          <>
            <button className="keyboard-help-btn save" onClick={handleSaveClick} disabled={!recordedKeys?.key}>{t('keyboardShortcuts.save')}</button>
            <button className="keyboard-help-btn cancel" onClick={() => { setIsEditing(false); setRecordedKeys(null); }}>{t('keyboardShortcuts.cancel')}</button>
          </>
        ) : (
          <>
            <button className="keyboard-help-btn modify" onClick={() => { setIsEditing(true); setRecordedKeys(null); }} title={t('keyboardShortcuts.modify')}>
              <FontAwesomeIcon icon={faPencilAlt} />
            </button>
            <button className="keyboard-help-btn reset" onClick={() => onReset(item.action)} title={t('keyboardShortcuts.reset')}>
            ...
            <button 
              ...
              disabled={isToggling}
              title={item.isEnabled ? t('keyboardShortcuts.disable') : t('keyboardShortcuts.enable')}
            >
