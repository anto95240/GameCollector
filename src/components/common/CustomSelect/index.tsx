...
const CustomSelect: React.FC<CustomSelectProps> = ({ options = [], value, onChange, disabled = false, error, touched, name, onBlur }) => {
  ...
  const handleToggle = () => {
    ...
    setIsOpen((prev) => {
      if (prev && onBlur) onBlur(name, value); // on blur when closing without selection
      return !prev;
    });
  };
