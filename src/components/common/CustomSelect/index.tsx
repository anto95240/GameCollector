...
const CustomSelect: React.FC<CustomSelectProps> = ({ options = [], value, onChange, disabled = false }: any) => {
  ...
  const handleSelect = (event: React.MouseEvent, val: string | number) => {
    if (!disabled) {
      onChange(val)
      setIsOpen(false)
    }
        ...
        >
          {options.map((opt: any) => (
            <div
              ...
              onClick={(e: any) => handleSelect(e, opt.value)}
            >
              {opt.label}
            </div>
          ))}
