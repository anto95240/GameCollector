export const normalizeText = (value: any) =>
  String(value || '')
    ...
    .replace(/[\u0300-\u036f]/g, '')
    .toLowerCase()
export const evaluateAchievementCondition = (achievement: any, stats: any) => {
  const idName = normalizeText(
    achievement?.id_name || achievement?.idName || achievement?.slug || achievement?.title
  )
