...
export const extractGamesList = (data) => (Array.isArray(data) ? data : data?.games || [])
export const mapGameWithMetadata = (game, metadata) => {
  ...
  return {
    ...game,
    genre:
      metadata.genres?.find((g) => g._id === (game.genre_id?._id || game.genre_id))?.genre_name ||
      'Inconnu',
    platform:
      metadata.platforms?.find((p) => p._id === (game.platform_id?._id || game.platform_id))
        ?.platform_name || 'Inconnu',
    status:
      metadata.statuses?.find((s) => s._id === (game.status_id?._id || game.status_id))
        ?.status_name || 'Inconnu',
    tags:
      game.tags_ids?.map(
        (t) => metadata.tags?.find((mt) => mt._id === (t._id || t))?.tag_name || 'Tag'
      ) || [],
  }
}
