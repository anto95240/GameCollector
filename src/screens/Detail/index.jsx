      ...
      try {
        ...
        if (id) {
          ...
          const gamesList = Array.isArray(gamesData)
            ? gamesData
            : gamesData.games || [];
          fetchedGame = gamesList.find(
            (g) =>
              g.name
                .toLowerCase()
                .replace(/ /g, "-")
                .replace(/[^\w-]+/g, "") === (slug || gameName),
          );
        }

        if (fetchedGame) {
          const genre = meta.genres?.find(
            (g) =>
              g._id === (fetchedGame.genre_id?._id || fetchedGame.genre_id),
          )?.genre_name;
          const platform = meta.platforms?.find(
            (p) =>
              p._id ===
              (fetchedGame.platform_id?._id || fetchedGame.platform_id),
          )?.platform_name;
          const status = meta.statuses?.find(
            (s) =>
              s._id === (fetchedGame.status_id?._id || fetchedGame.status_id),
          )?.status_name;
          const tags =
            fetchedGame.tags_ids?.map((tagId) => {
              const found = meta.tags?.find(
                (t) => t._id === (tagId?._id || tagId),
              );
          ...
          setGame({
            ...
            playtime: fetchedGame.playing_time
              ? `${fetchedGame.playing_time}h`
              : "N/A",
            achievements: fetchedGame.succes || "N/A",
            userNotes: fetchedGame.comment || "",
            addedDate: new Date(fetchedGame.createdAt).toLocaleDateString(
              "fr-FR",
            ),
            image: fetchedGame.image?.startsWith("http")
              ? fetchedGame.image
              : `${import.meta.env.VITE_API_URL}${fetchedGame.image}`,
          });
          ...
          try {
            ...
            console.error(
              "Erreur lors de la sauvegarde de l'historique BDD",
              err,
            );
          }
        }
