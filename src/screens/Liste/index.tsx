...
const ListePage = () => {
  ...
  const filterData = useMemo(() => {
    ...
    const years = Array.from(new Set(games.map((g: any) => Number(g.year)).filter((y) => !Number.isNaN(y))))
      .sort((a, b) => b - a)
      .map(String);
    const ratings = Array.from(new Set(games.map((g: any) => g.rating).filter((r) => r !== undefined && r !== null))).sort((a, b) => b - a).map(String);
  ...
  const handleToggleFavorite = useCallback((g: any) => {
    setActiveMenuIndex(null);
    toggleFavorite(g);
  }, [toggleFavorite]);
  ...
  const handleScrollRight = useCallback((e: any) => {
    e.stopPropagation();
    scroll("right");
  }, [scroll]);
    ...
    <div className="liste-page-container w-full flex flex-col" onClick={() => setActiveMenuIndex(null)}>
      ...
      <div className="main-stage">
        {isLoading && games.length === 0 ? (
          <SkeletonList count={viewMode === 'grid' ? 8 : 4} className="w-full mt-12 mb-8" />
        ) : viewMode === 'grid' ? (
          ...
          <div ref={gridContainerRef} className="vgrid-stage">
            {sortedGames.length > 0 ? (
              <VirtualGameGrid
                ...
                itemHeight={260}
                activeTab={activeTab}
                activeMenuIndex={activeMenuIndex ?? undefined}
