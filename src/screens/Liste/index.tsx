...
const ListePage = () => {
  ...
  // Activation auto de l'onglet wishlist si naviguée depuis la navbar
  useEffect(() => {
    if (location.state?.tab === 'wishlist') {
      setActiveTab('wishlist')
      // Nettoyer le state pour éviter de re-activer au refresh
  ...
  const filterData = useMemo(() => {
    ...
    const years = Array.from(
      new Set(games.map((g: any) => Number(g.year)).filter((y) => !Number.isNaN(y)))
    )
      .sort((a, b) => b - a)
      .map(String)
    const ratings = Array.from(
      new Set(games.map((g: any) => g.rating).filter((r) => r !== undefined && r !== null))
    )
      .sort((a, b) => b - a)
      .map(String)
