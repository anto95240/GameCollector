class KeyboardShortcutsService {
  ...
  loadCustomBindings(userShortcuts: any[]) {
    this.customBindings.clear()
    if (Array.isArray(userShortcuts)) {
      userShortcuts.forEach((shortcut: any) => {
  ...
  _getActiveBinding(action: string) {
  ...
  handleKeyDown = (event: KeyboardEvent) => {
    ...
    for (const [action] of this.shortcuts.entries()) {
      ...
      if (this._matchesShortcut(event, activeBinding.key, activeBinding.options)) {
  ...
  getReadableShortcut(action: string) {
